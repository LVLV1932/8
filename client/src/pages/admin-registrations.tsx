    ...
    (async () => {
      ...
      if (!res.ok) {
        toast({
          variant: "destructive",
          title: "خطأ",
          description: payload?.message || "تعذر تحميل الطلبات",
        });
        return;
      }
  ...
  const handleApprove = async (id: string) => {
    ...
    if (!res.ok) {
      toast({
        variant: "destructive",
        title: "خطأ",
        description: payload?.message || "تعذر قبول الطلب",
      });
      return;
    }
  ...
  const handleReject = async (id: string) => {
    ...
    if (!res.ok) {
      toast({
        variant: "destructive",
        title: "خطأ",
        description: payload?.message || "تعذر رفض الطلب",
      });
      return;
    }
    setRegistrations((prev) => prev.filter((r) => r.id !== id));
    ...
    <div className="space-y-6">
      <div className="flex gap-4 flex-wrap">
        ...
        <div className="space-y-2">
          ...
          <div className="border rounded-lg p-2 text-sm text-muted-foreground">
            في الانتظار
          </div>
        </div>
      </div>
